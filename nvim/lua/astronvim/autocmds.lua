...
-- -- 保存时自动格式化
-- autocmd("BufWritePre", {
--   group = augroup("file_format", { clear = true }),
--   pattern = { "*.lua", "*.py", "*.sh" },
--   callback = function()
--     vim.lsp.buf.format()
--   end,
...
--
autocmd("InsertLeave", {
  group = augroup("file_output", { clear = true }),
  -- callback = function() utils.cmd "echo 11111 >> ~/1.txt" end,
  callback = require("astronvim.utils.im-select").macInsertLeave,
})
autocmd("InsertEnter", {
  group = augroup("file_input", { clear = true }),
  -- callback = function() utils.cmd "echo 22222 >> ~/1.txt" end,
  callback = require("astronvim.utils.im-select").macInsertEnter,
})
